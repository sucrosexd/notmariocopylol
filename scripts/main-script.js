...
function init() {
    ...
    platforms = [
        {x: 0, y: levelHeight - 30, width: levelWidth, height: 30, color: '#27AE60'}, // земля
...
function updateCamera() {
    ...
    // Обновление индикатора камеры
    cameraIndicator.textContent = `Камера: ${Math.round(camera.x)}/${levelWidth}`;
}
...
function handleKeyboardInput() {
    ...
    if (keys['Escape'] || keys['KeyP']) {
        ...
        // Сбрасываем состояние клавиши паузы, чтобы избежать многократного переключения
        keys['Escape'] = false;
        keys['KeyP'] = false;
    }
}
